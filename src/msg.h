...
const char* MCMDLINE = "👓 I need either a command to run or a PID to attach to";

const char* MMALLOC = "😵 Failed to allocate the memory for dumping frame stacks. This is pretty bad\n"
                      "as Austin doesn't require much memory to run. Try to free up some memory";

const char* MTIMEOUT = "👽 The process you are trying to %s doesn't seem to be a Python process. If you\n"
...
#endif
    URL("https://github.com/P403n1x87/austin#on-macos-1") "for more details";
#endif // PL_UNIX

const char* MFORK = "❌ Cannot launch the given command or it terminated too quickly";

const char* MATTACH = "🛑 Cannot attach to the given process. Make sure that the PID you have provided\n"
                      "is valid and that the process is still running";

const char* MERROR = "☠️ An unexpected error occurred. Please report the issue to\n" URL(
    "https://github.com/P403n1x87/austin/issues"
) "and attach the logs generated by Austin";

const char* MNOPROC = "😵 No such process. Something weird just happened. If this happens again,\n"
                      "please open an issue at\n" URL("https://github.com/P403n1x87/austin/issues");

const char* MNOPYTHON = "👾 It looks like you are trying to profile a process that is not a Python\n"
...
const char* MNOCHILDREN = "🚼 It looks like you are trying to profile a process that is not a Python\n"
